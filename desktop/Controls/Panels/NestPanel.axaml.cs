...
public partial class NestPanel : UserControl
{
    ...
    private void OnNestClick(object? s, RoutedEventArgs e)
    {
        ...
        var (placed, skipped, warnings) = Vm.Nest(margin, spacing, rotStep);
        TbNestResult.IsVisible = true;
        TbNestResult.Text = skipped == 0
            ? $"Nested {placed} part(s)."
            : $"Nested {placed} part(s), skipped {skipped}.{(warnings.Count > 0 ? " " + string.Join("; ", warnings) : "")}";
    }
}
